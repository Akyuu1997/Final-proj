...
renderPlot({
 ...
 #样本画图，x和y的lim由轴决定
  ...
  functionRev = function(d,Status,S1,S2,MRG=Input$MRG){
    B0  = subset(B,B$Status==Status)
    Buy = mean(B0$Buy)
    Rev = sum(B0$Rev[B0$Rev<12000])#同上
    Rev*(1-d)*max(Buy*(1+S1*d),1)*(1+S2*d)      #小组的Rev
  }


  for(i in length(s)) {
    Rev = sapply(d, functionRev, Status=S[i], S1=S1[i],S2=S2[i])
    nd = d[which.max(PI)]
...
```{r}
inputPanel(
    sliderInput("Y1", "Y軸", -6, 6, c(-6,7), 1),
    sliderInput("Y2", "Y軸", 0, 2400, c(-5,8),1),
